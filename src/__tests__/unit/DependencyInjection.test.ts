...
describe('DependencyInjection service', function () {
    ...
    function getClass (externalDependencies?: ExternalDependencies): DependencyInjection {
    ...
    it(
        'Should check external dependencies error ' +
        'when try to find a service we didn\'t register',
        function () {
            const dependencyInjection = getClass();
    ...
    it('Should check editing the Registered Service Descriptions', function () {
        const dependencyInjection = getClass();
        const serviceName = 'newService';
        const serviceName2 = 'newService2';
        ...
        chai.assert.deepEqual(
            dependencyInjection.get(serviceName).name,
            dependencyInjection.get(serviceName2).otherService.name
        );
    });

    it(
        'Should create two instances of dependency injection with external dependencies and' +
        ' registering same objects different names',
        function () {
            const externalDependencies = new ExternalDependencies();
    ...
    it(
        'Should throw errors when we try to register a service with @ as first character',
        function () {
            const dependencyInjection = getClass();
    ...
    it(
        'Should throw errors when we try to register a service with an empty name',
        function () {
            const dependencyInjection = getClass();
    ...
    it('Should do recursion of dependencies', function () {
        ...
        class NewToInject {
            ...
            public injectDependence <T> (newService: T): void {
                chai.assert.deepEqual(newService, dependencyInjection.get('newService'));
        ...
        dependencyInjection.registerService(
            'newToInject', ['newService'],
            NewToInject,
            true,
            NewToInject.prototype.injectDependence
        );
        ...
        chai.assert.deepEqual(
            ...
            dependencyInjection.get('newToInject').newService
        );
        chai.assert.deepEqual(
            dependencyInjection.get('newToInject'),
            dependencyInjection.get('newService').newToInject
        );
    });
    ...
    it('Should register classes using external register', function () {
        const dependencyInjection = getClass();
        const externalServicesRegister = new ExternalServicesRegister();
    ...
    it('Should register classes using two external registers', function () {
        const dependencyInjection = getClass();
        const externalServiceRegister1 = new ExternalServicesRegister();
        const externalServiceRegister2 = new ExternalServicesRegister();
    ...
    it(
        'Should create two services with same name in different registers and throw error',
        function () {
            const dependencyInjection = getClass();
            const externalServiceRegister1 = new ExternalServicesRegister();
            const externalServiceRegister2 = new ExternalServicesRegister();
            const serviceName = 'newToInject';
